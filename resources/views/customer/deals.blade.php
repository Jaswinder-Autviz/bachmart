...
<div class="container py-4">
    ...
    <div class="d-flex align-items-center justify-content-between mb-4 flex-wrap gap-2">
        ...
        @if(request()->hasAny(['category','city','min_discount','sort','featured']))
            <a href="{{ route('deals') }}" class="btn btn-outline-danger btn-sm rounded-pill px-3">
        ...
        <div class="col-lg-3">
            <div class="card border-0 shadow-sm rounded-4 sticky-top p-3 bg-white" style="top:95px">
                <h6 class="fw-800 mb-3 pb-2 border-bottom d-flex align-items-center justify-content-between">
                    <span><i class="bi bi-funnel-fill text-primary-bm me-1"></i>Filter Deals</span>
                    @if(request()->hasAny(['category','city','min_discount','sort','featured']))
                        <a href="{{ route('deals') }}" class="small text-danger text-decoration-none">Clear</a>
                    @endif
                </h6>
                ...
                <form method="GET" action="{{ route('deals') }}" id="filterForm">
                    {{-- Sort --}}
                    <div class="mb-3">
                        <label class="form-label small fw-700">Sort By</label>
                    ...
                    <div class="mb-3 form-check">
                        ...
                        <label class="form-check-label small fw-600" for="featuredCheck">⭐ Featured Deals Only</label>
                    </div>

                    <button type="submit" class="btn btn-primary-bm btn-sm w-100 mb-2">Apply Filters</button>
                    @if(request()->hasAny(['category','city','min_discount','sort','featured']))
                        <a href="{{ route('deals') }}" class="btn btn-outline-secondary btn-sm w-100">Clear All</a>
                    @endif
                </form>
            </div>
        </div>
        ...
        <div class="col-lg-9">
            ...
            @else
                <div class="card border-0 shadow-sm rounded-4 p-5 text-center bg-white">
                    ...
                    <h5 class="fw-800 text-dark">No clearance deals found</h5>
                    <p class="text-muted small mb-3">Try removing some filters to see more results.</p>
                    <div>
                        <a href="{{ route('deals') }}" class="btn btn-primary-bm btn-sm px-4">Clear Filters</a>
                    </div>
                </div>
            @endif
        </div>
