...
<div class="card-admin p-3 mb-4">
    <form method="GET" action="{{ route('admin.shops.index') }}" class="d-flex gap-2 flex-wrap align-items-center">
        <input type="text" name="search" class="form-control form-control-sm rounded-pill" placeholder="Shop name, city..." value="{{ request('search') }}" style="max-width:240px">
                ...
                <tr>
                    <td class="ps-3">
                        <div class="d-flex align-items-center gap-2.5">
                            <img src="{{ $shop->logo_url }}" width="38" height="38" class="rounded-3 border" style="object-fit:cover;background:#F8FAFC">
                            <div>
                                ...
                                <div class="d-flex gap-1 mt-0.5">
                                    @if($shop->is_verified)<span class="badge bg-success-subtle text-success rounded-pill" style="font-size:.65rem">Verified</span>@endif
                    ...
                    <td class="text-end pe-3">
                        <div class="d-flex gap-1 justify-content-end align-items-center">
                            <a href="{{ route('admin.shops.show', $shop) }}" class="btn btn-sm btn-outline-secondary py-1 px-2 rounded-pill" style="font-size:.75rem"><i class="bi bi-eye"></i></a>
                            @if($shop->status !== 'active')
                            <form action="{{ route('admin.shops.activate', $shop) }}" method="POST">@csrf<button class="btn btn-sm btn-success py-1 px-2.5 rounded-pill" style="font-size:.75rem">Activate</button></form>
                            @else
                            <form action="{{ route('admin.shops.deactivate', $shop) }}" method="POST">@csrf<button class="btn btn-sm btn-outline-secondary py-1 px-2.5 rounded-pill" style="font-size:.75rem">Deactivate</button></form>
                            @endif
                            @if(!$shop->is_verified)
                            <form action="{{ route('admin.shops.verify', $shop) }}" method="POST">@csrf<button class="btn btn-sm btn-primary py-1 px-2.5 rounded-pill" style="font-size:.75rem">Verify</button></form>
                            @endif
                        </div>
                    </td>
                </tr>
